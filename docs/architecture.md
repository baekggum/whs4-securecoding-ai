...
## 11. [Sprint 1] 배포 토폴로지 (Docker Compose)

작성자: 지훈 (DevOps). 구성 파일: 루트 `docker-compose.yml`, `backend/Dockerfile`,
`backend/docker-entrypoint.sh`, `frontend/Dockerfile`, `frontend/nginx.conf`, 루트 `.env.example`.
운영 절차/검증 체크리스트는 `docs/deploy-handoff.md` 참고.

### 11.1 토폴로지

```
                        호스트 :8080 (HTTP_PORT, 유일한 공개 포트)
                                 │
                                 ▼
┌────────────────────────────────────────────────────────┐
│  frontend  (nginx:1.27-alpine)                          │
│  - Vite 프로덕션 빌드 정적 서빙 (+ SPA fallback)            │
│  - /api        → http://backend:4000  (헤더: XFF/XFP)    │
│  - /uploads    → http://backend:4000                    │
│  - /socket.io  → http://backend:4000  (Upgrade/Connection│
│                   헤더로 WebSocket 통과, 폴링도 지원)        │
└───────────────┬────────────────────────────────────────┘
                │ docker 내부 네트워크 (호스트 미노출)
                ▼
┌────────────────────────────────────────────────────────┐
│  backend  (node:20-bookworm-slim, USER node)            │
│  - entrypoint: prisma migrate deploy → node dist/server │
│  - Express + Socket.IO :4000, trust proxy=1             │
│  - 세션: connect-pg-simple (session 테이블 자동 생성)       │
│  - /app/uploads ←─ named volume `uploads-data`          │
└───────┬────────────────────────────────────────────────┘
        │
        ▼
┌──────────────────────────────┐
...
- **단일 오리진 강제**: 브라우저는 오직 frontend(nginx) 오리진 하나만 본다. `/api`, `/uploads`,
  `/socket.io`를 nginx가 프록시하므로 개발 시 Vite 프록시(§`frontend/vite.config.ts`)와 동형이고,
  README의 "같은 registrable domain" 요건(SameSite=Lax 세션 쿠키)이 자동으로 충족된다.
  `VITE_API_BASE_URL`은 빈 값으로 빌드(상대 URL 사용).
- **기동 순서**: postgres healthcheck 통과 → backend entrypoint가 `prisma migrate deploy`
  (실패 시 3초 간격 5회 재시도) → 서버 기동 → backend `/health/ready` healthcheck(DB `SELECT 1` 포함) 통과 → frontend 기동.
  스키마가 항상 코드보다 먼저 최신화되는 것을 compose 레벨에서 보장.
- **네이티브 모듈(sharp/bcrypt) 플랫폼 일치**: backend Dockerfile의 deps/build/prod-deps/runtime
  네 스테이지가 전부 `node:20-bookworm-slim`을 사용 — 프리빌드 바이너리가 항상 런타임과 같은
  glibc/linux 플랫폼에서 설치된다(README "Windows에서 실행 시" 항목의 사고를 이미지 레벨에서 차단).
  호스트의 `node_modules`는 `.dockerignore`로 컨텍스트에서 제외.
- **런타임 이미지 최소화 + migrate 가능**: 런타임에는 `--omit=dev` 의존성만 담되, 마이그레이션에
  필요한 prisma CLI(`node_modules/prisma` + `@prisma/*` + 생성된 `.prisma` 클라이언트)만 빌드
  스테이지에서 선별 복사. tsc/tsx/@types는 이미지에 없음.
- **빌드 컨텍스트는 저장소 루트**: npm workspaces 단일 lockfile(`package-lock.json`이 루트에만 존재)
  때문에 `npm ci --workspace <ws>`는 루트에서만 재현 가능. 두 Dockerfile 모두 이를 전제로 한다.
- **감독(supervisor)**: 서버는 `uncaughtException` 시 의도적으로 종료하므로(README, REPORT.md §14)
  compose `restart: unless-stopped`가 자동 재기동을 담당. 종료 시 SIGTERM이 PID 1인 node에 직접
  전달되도록 entrypoint는 `exec node dist/server.js`로 끝난다(그레이스풀 셧다운 §`server.ts`).
- **영속 데이터**: DB(`postgres-data`)와 상품 이미지(`uploads-data`)만 볼륨. 그 외는 모두 무상태 —
  컨테이너 재생성 안전. 세션도 DB에 있으므로(§5) backend 재시작이 로그인 상태를 날리지 않는다.
- **확장 한계(기존 §5 명시 사항 재확인)**: `user:dormant` 즉시 무효화가 인프로세스 EventEmitter
  기반이라 backend는 단일 인스턴스 전제. 수평 확장 시 Socket.IO Redis adapter 도입이 선행되어야
  하며, 이 compose는 의도적으로 backend 1대 구성이다.

### 11.3 HTTPS 배포 시

TLS는 이 compose 바깥(예: Caddy/Traefik/클라우드 LB)에서 종료하고 frontend `:80`으로 전달한다.
그때 `.env`에서 `PUBLIC_ORIGIN=https://<도메인>`, `COOKIE_SECURE=true`로 변경하면 nginx가 전달하는
`X-Forwarded-Proto`와 backend의 `trust proxy` 설정(§`app.ts`)에 의해 Secure 쿠키가 정상 동작한다.
평문 HTTP에서 `COOKIE_SECURE=true`는 로그인 불능을 유발하므로 금지(§`env.ts` 주석).
