...
walletRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await walletService.transfer(requireCurrentUser(req).id, receiverId, amount, idempotencyKey);
...
walletRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const transactions = await walletService.listTransactions(requireCurrentUser(req).id, { before, limit, direction });
